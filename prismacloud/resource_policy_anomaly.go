...
func resourceAnomalyPolicy() *schema.Resource {
	return &schema.Resource{
		Create: createUpdateAnomalyPolicy,
		Read:   readAnomalyPolicy,
		Update: createUpdateAnomalyPolicy,
		Delete: deleteAnomalyPolicy,
...
func parseAnomalyPolicy(d *schema.ResourceData, id string) policy.Policy {
...
func saveAnomalyPolicy(d *schema.ResourceData, obj policy.Policy) {
	d.Set("policy_id", obj.PolicyId)
	d.Set("name", obj.Name)
	d.Set("policy_type", obj.PolicyType)
	d.Set("policy_sub_types", obj.PolicySubTypes)
	d.Set("system_default", obj.SystemDefault)
	d.Set("policy_upi", obj.PolicyUpi)
...
func createUpdateAnomalyPolicy(d *schema.ResourceData, meta interface{}) error {
	var err error
	client := meta.(*pc.Client)

	id := d.Id()
	if id == "" {
		id, err = policy.Identify(client, d.Get("name").(string))
		if err != nil {
			return err
		}
	}

	obj := parseAnomalyPolicy(d, id)

	// Copy over the RO fields from the live object.
	liveObj, err := policy.Get(client, id)
	if err != nil {
		return err
	}
	obj.PolicySubTypes = make([]string, len(liveObj.PolicySubTypes))
	copy(obj.PolicySubTypes, liveObj.PolicySubTypes)
	obj.PolicyUpi = liveObj.PolicyUpi
	...
	d.SetId(id)
	return readAnomalyPolicy(d, meta)
}

func readAnomalyPolicy(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*pc.Client)
	id := d.Id()
	...
	saveAnomalyPolicy(d, obj)

	return nil
}

func deleteAnomalyPolicy(d *schema.ResourceData, meta interface{}) error {
	d.SetId("")
	return nil
}
