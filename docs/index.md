...
* `max_retries` -  Maximum number of times an API call is retried when requests are throttled.
* `retry_max_delay` - Maximum time the API calls are retried when creating or updating resources.
