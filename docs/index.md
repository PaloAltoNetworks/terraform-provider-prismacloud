...
* `max_retries` - (Optional) Defaults to 0. Maximum number of times an API call is retried when requests are throttled (default: `5`). 
* `retry_max_delay` - (Optional) Defaults to 0. Maximum time the API calls are retried when creating or updating resources (default: `30`). 
